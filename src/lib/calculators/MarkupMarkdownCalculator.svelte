<script>
    ...
    const calculationTypes = [
        ...
        { value: 'selling-from-markup', label: 'Calculate Selling Price from Markup' },
        { value: 'markdown', label: 'Calculate Markdown Percentage' },
        { value: 'sale-from-markdown', label: 'Calculate Sale Price from Markdown' }
    ];
    ...
    function calculate() {
        switch (calculationType) {
            case 'markup':
                ...
                resultPercentage = ((sellingPrice - costPrice) / costPrice) * 100;
            ...
            case 'markdown':
                if (costPrice <= 0 || sellingPrice <= 0) return;
                resultPercentage = ((costPrice - sellingPrice) / costPrice) * 100;
                profit = sellingPrice - costPrice;
    ...
    <Card>
        <div class="p-6">
            ...
            <p class="text-gray-600 mb-6">
                Calculate markup and markdown percentages for pricing strategies. 
                Perfect for retail, business, and sales price optimization.
            </p>
            ...
            <div class="p-6 text-center">
                {#if calculationType === 'markup' || calculationType === 'markdown'}
                    ...
                    <div class="text-xl text-gray-600">
                        {calculationType === 'markup' ? 'Markup Percentage' : 'Markdown Percentage'}
                    </div>
                {:else}
                    ...
                    <div class="text-xl text-gray-600">
                        {calculationType === 'selling-from-markup' ? 'Selling Price' : 'Sale Price'}
                    </div>
                {/if}
            </div>
        </Card>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            <span class="text-gray-600">
                                {calculationType.includes('markdown') ? 'Original Price:' : 'Cost Price:'}
                            </span>
                            <span class="font-semibold">{formatCurrency(costPrice)}</span>
                        </div>
                        
                        {#if calculationType === 'markup' || calculationType === 'markdown'}
                            <div class="flex justify-between">
                                <span class="text-gray-600">
                                    {calculationType === 'markup' ? 'Selling Price:' : 'Sale Price:'}
                                </span>
                                <span class="font-semibold">{formatCurrency(sellingPrice)}</span>
                            </div>
                        {:else}
                            <div class="flex justify-between">
                                <span class="text-gray-600">
                                ...
                                <span class="font-semibold text-green-600">{formatCurrency(resultPrice)}</span>
                            </div>
                        {/if}

                        <div class="flex justify-between">
                            <span class="text-gray-600">
                                {calculationType.includes('markup') ? 'Markup:' : 'Markdown:'}
                            </span>
                            <span class="font-semibold text-blue-600">
                                {calculationType === 'markup' || calculationType === 'markdown' 
                                    ? formatNumber(resultPercentage, 2) + '%'
                                    : (calculationType === 'selling-from-markup' ? markupPercentage : markdownPercentage) + '%'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold {profitMargin >= 0 ? 'text-green-600' : 'text-red-600'}">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold">
                                {formatCurrency(Math.abs(profit))} {profit >= 0 ? 'gain' : 'loss'}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            <h4 class="font-semibold text-blue-900 mb-2">Markup Percentage</h4>
                            <div class="text-blue-800 text-sm">
                                <div class="font-mono">Markup % = ((Selling Price - Cost) / Cost) × 100</div>
                                <div class="mt-1">Based on cost price</div>
                            </div>
                        </div>

                        <div class="p-4 bg-green-50 border border-green-200 rounded-lg">
                            <h4 class="font-semibold text-green-900 mb-2">Selling Price from Markup</h4>
                            <div class="text-green-800 text-sm">
                                <div class="font-mono">Selling Price = Cost × (1 + Markup%/100)</div>
                    ...
                    <div class="space-y-4">
                        <div class="p-4 bg-purple-50 border border-purple-200 rounded-lg">
                            <h4 class="font-semibold text-purple-900 mb-2">Markdown Percentage</h4>
                            <div class="text-purple-800 text-sm">
                                <div class="font-mono">Markdown % = ((Original - Sale) / Original) × 100</div>
                                <div class="mt-1">Based on original price</div>
                            </div>
                        </div>

                        <div class="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                            <h4 class="font-semibold text-orange-900 mb-2">Sale Price from Markdown</h4>
                            <div class="text-orange-800 text-sm">
                                <div class="font-mono">Sale Price = Original × (1 - Markdown%/100)</div>
                                <div class="mt-1">Subtract markdown from original</div>
                            </div>
                        </div>
                    </div>
                </div>
